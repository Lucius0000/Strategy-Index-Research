...
INDEX_TICKERS: List[str] = ['000300.SS']
# ETF 清单       
...
# 表格路径
PERIODS_TABLE_PATH: str = 'data/CSI300.xlsx'
# 指定 sheet
...
BRIEF_TXT = DIR_RAW / "CSI300_brief.txt"
...
def fetch_index_full(ticker: str) -> pd.DataFrame:
    sym = ak_symbol_for_index(ticker)
    return _normalize_price_columns(ak.stock_zh_index_daily(symbol=sym))

def fetch_etf_full_sina_first(ticker: str, adjust: str = "") -> pd.DataFrame:
    c = code6(ticker); ex = to_exchange_prefix(ticker); last_err = None
    for _ in range(MAX_RETRY):  # 新浪优先
        try:
            df = _normalize_price_columns(ak.fund_etf_hist_sina(symbol=f"{ex}{c}"))
            if not df.empty: return df
        except Exception as e:
            last_err = e; time.sleep(RETRY_SLEEP_SECONDS)
    for _ in range(MAX_RETRY):  # 回退东财
        try:
            df = _normalize_price_columns(ak.fund_etf_hist_em(
                symbol=c, period="daily", start_date=START_DATE_FALLBACK,
                end_date=END_DATE_TODAY, adjust=adjust))
            if not df.empty: return df
        except Exception as e:
            last_err = e; time.sleep(RETRY_SLEEP_SECONDS)
    raise RuntimeError(f"ETF {ticker} 下载失败：{last_err}")

# ETF 分红（累计）下载
def fetch_etf_dividend_cumulative(ticker: str) -> pd.Series:
    """
    读取 ETF 的累计分红（单位：元/份），返回以日期为索引的 float Series（升序）。
    非 ETF 返回空 Series。
    """
    if is_index_ticker(ticker):
        return pd.Series(dtype=float)
    c = code6(ticker); ex = to_exchange_prefix(ticker)
    df = ak.fund_etf_dividend_sina(symbol=f"{ex}{c}")
...
def fetch_full_history_by_ticker(ticker: str) -> pd.DataFrame:
    return fetch_index_full(ticker) if is_index_ticker(ticker) else fetch_etf_full_sina_first(ticker, ETF_ADJUST)
...
# ======= 新增：按“牛市/熊市/震荡”归一周期类型，并基于原始日度数据做分组汇总 =======
...
def summarize_metrics_by_cycle_type(
    ...
    """
    对某一标的 tk，在“指定周期类型”（牛市/熊市/震荡）下做汇总，核心原则：
    - 不直接对已年化指标做算术平均；
    - 而是拼接该类型下各区间的“原始日度数据”，计算整体的年化收益/波动/夏普/信息比率/跟踪误差等；
    - 最大回撤：按你的要求，取同类区间中“最差（最深）回撤”（即数值最小的那个回撤）；
    ...
    for start_str, end_str, ptype_raw in periods_ex:
        ...
        # 日度收益（区间内自行 pct_change，避免跨区间拼接产生“跨段收益”）
...
def main():
    ...
    # ======= 新增：按市场周期类型（牛市/熊市/震荡）汇总（基于原始日度数据） ======= #
    cycle_rows_cn = []
